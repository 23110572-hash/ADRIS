...
import { Ban, CirclePhoneCall, KeyRound, LogOut, ShieldAlert, WalletCards } from "lucide-react";
...
const actions = [
  ...
  { icon: CirclePhoneCall, title: "Contact a trusted channel now.", detail: "Call 1930, your bank using the number on its official website or card, or a trusted family member." },
];
  ...
  return (
    <PageShell eyebrow="Works without AI" title="Stop. You are not under ‘digital arrest’." description="These reviewed instructions are designed to remain available from the installed ADRIS PWA even when analysis services are unavailable.">
      <div className="rounded-3xl border-2 border-red-300 bg-red-50 p-5 sm:p-8">
        ...
        <div className="mt-6 flex flex-wrap gap-3"><Button asChild variant="destructive" size="lg"><a href="tel:1930"><CirclePhoneCall /> Call 1930</a></Button><Button asChild variant="outline" size="lg"><Link href="/check/message">Check submitted evidence</Link></Button><Button asChild variant="outline" size="lg"><Link href="/reporting">Reporting steps</Link></Button></div>
      </div>
